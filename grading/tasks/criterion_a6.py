...
def task_A06_03(task: Task) -> Result:
    ...
    try:
        ...
        if "dmz.worldskills.org" in cmd_result.result:
            ...
            score += 1
        command = "dig +tcp +short +time=2 +tries=1 @1.1.1.10 dmz.worldskills.org SOA"
        cmd_result = task.run(task=paramiko_command, command=command)
        if "dmz.worldskills.org" in cmd_result.result:
            msg = (
...
def task_A06_04(task: Task, cheated: bool) -> Result:
    """SNAT check"""
    if cheated:
        return Result(
            host=task.host,
            msg="SNAT is not working on fw01 as default route exists on jamie-ws01",
            command_run="ip route",
            score=0,
            max_score=0.1,
        )
