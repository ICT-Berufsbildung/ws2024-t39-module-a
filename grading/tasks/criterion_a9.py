...
def task_A09_03(task: Task) -> Result:
    ...
    # Send mail over SMTP
    command = "printf 'Subject: WSC2024_FLAG\n\nWSC2024_FLAG'  | curl -k --ssl-reqd  --url smtps://localhost --user jamie:Skill39 -H 'Subject: WSC2024_FLAG' --mail-from 'jamie.oliver@dmz.worldskills.org(Jamie Oliver)' --mail-rcpt jamie.oliver@dmz.worldskills.org --upload-file -"
    ...
    cmd_result = None
    command = r"""IFS=" " read -r -a mail_ids <<< "$(curl -k imaps://jamie:Skill39@localhost/INBOX?SUBJECT%20WSC2024_FLAG 2>&1 | grep -oP '(?<=SEARCH)[ 0-9]+')" && curl -k "imaps://jamie:Skill39@localhost/INBOX;MAILINDEX=${mail_ids[-1]}" 2>&1"""
    commands.append(command)
    try:
        cmd_result = task.run(task=paramiko_command, command=command)
        command_outputs.append(cmd_result.result)
...
def task_A09_04(task: Task) -> Result:
    """test echo service"""
    score = 0
    command = "printf 'Subject: WSC2024_ECHO_FLAG\nFrom: jamie.oliver@dmz.worldskills.org\nTo: echo@dmz.worldskills.org\n\nWSC2024_ECHO_FLAG' | curl -k --ssl-reqd  --url smtps://localhost --user jamie:Skill39 --mail-from 'jamie.oliver@dmz.worldskills.org(Jamie Oliver)' --mail-rcpt echo@dmz.worldskills.org --upload-file -"
    commands = [command]
    command_outputs = []
    ...
    cmd_result = None
    command = r"""IFS=" " read -r -a mail_ids <<< "$(curl -k imaps://jamie:Skill39@localhost/INBOX?FROM%20echo@dmz.worldskills.org 2>&1 | grep -oP '(?<=SEARCH)[ 0-9]+')" && curl -k "imaps://jamie:Skill39@localhost/INBOX;MAILINDEX=${mail_ids[-1]}" 2>&1"""
    commands.append(command)
    try:
        cmd_result = task.run(task=paramiko_command, command=command)
        command_outputs.append(cmd_result.result)
