...
def task_A1_02(task: Task) -> Result:
    ...
    try:
        ...
        cmd_result = task.run(task=paramiko_command, command=f'({command} || {base_command}) 2>&1')
        # Check if ou exits
        got_mark = True if "dn: ou=Employees,dc=int,dc=worldskills,dc=org" in cmd_result.result else False
    except:
        got_mark = False
    ...
    return Result(
        host=task.host,
        id="A1_02",
        result=f"OU Employees exists" if got_mark else "OU Employees DOES NOT exists",
        command_run=command,
        score=0.1 if got_mark else 0.0,
        max_score=0.1
    )
...
def task_A1_03(task: Task) -> Result:
    ...
    if login_works and cn:
        ...
        msg = f"User jamie exists, but attributes are missing: {', '.join(missing_attr)}"
    else:
        msg = "User jamie DOES NOT exists"
...
def task_A1_04(task: Task) -> Result:
    ...
    if login_works and cn:
        ...
        msg = f"User peter exists, but attributes are missing: {', '.join(missing_attr)}"
    else:
        msg = "User peter DOES NOT exists"
...
def task_A1_05(task: Task) -> Result:
    ...
    if login_works and cn:
        ...
        msg = f"User admin exists, but attributes are missing: {', '.join(missing_attr)}"
    else:
        msg = "User admin DOES NOT exists"
